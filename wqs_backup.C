#include "TFile.h"
#include "TROOT.h"
#include "TH1F.h"
#include "TString.h"
#include "TLegend.h"
#include <iostream>

void wqs_backup(){
	double factors[7]={0.308238,0.45956,0.589088,0.701125,0.684967,0.664385,0.583014};
	double factors_err[7]={0.0299281,0.0251473,0.0250306,0.00803506,0.0230339,0.0303196,0.027369};
	//Luminositaeten

	double L[7];
	L[0]=675.8590;
	L[1]=543.6270;
	L[2]=419.7760;
	L[3]=3122.204;
	L[4]=639.8380;
	L[5]=479.2400;
	L[6]=766.8380;
	double L_err[7]={5.721257,4.830643,3.974844,22.31760,5.577354,4.481870,6.497519};
	//Schwerpunktsenergien
	double S[7]={88.48021,89.47158,90.22720,91.23223,91.97109,92.97091,93.71841};
	//Strahlungskorrekturen
	double wq_cor_lep[7]={0.09,0.2,0.36,0.52,0.22,-0.01,-0.08};//leptonisch
	double wq_cor_had[7]={2.0,4.3,7.7,10.8,4.7,-0.2,-1.6};//hadronisch
	//Eventnamen
	TString names[4]={"ee","mm","TT","qq"};
	//Anzahl Ereignisse nach Schnitt,
	//sortiert nach Schwerpunktsenergie
	double N[4][7];
	//e-events
	N[0][0]=908;
	N[0][1]=823;
	N[0][2]=719;
	N[0][3]=6442;
	N[0][4]=981;
	N[0][5]=484;
	N[0][6]=713;
	//m-events
	N[1][0]=137;
	N[1][1]=242;
	N[1][2]=332;
	N[1][3]=3905;
	N[1][4]=691;
	N[1][5]=269;
	N[1][6]=329;
	//t-events
	N[2][0]=158;
	N[2][1]=222;
	N[2][2]=277;
	N[2][3]=3735;
	N[2][4]=619;
	N[2][5]=280;
	N[2][6]=303;
	//q-events
	N[3][0]=3528;
	N[3][1]=5322;
	N[3][2]=7542;
	N[3][3]=92764;
	N[3][4]=15294;
	N[3][5]=6643;
	N[3][6]=7419;
	//invertierte effizienzmatrizen
	double ineps[4][4]= {
{2.05485, -0.00036755, -0.00670764, -0.000108392},
{0.000544692, 1.10921, -0.0292035, 0.000190725},
{-0.0220851, -0.00736568, 1.08768, -0.00754559},
{-0.000703096, 1.62783e-007, -0.00168098, 1.01042}
};

//fehler auf ineps
	double ineps_err[4][4]={
{0.00687823, 9.90757e-005, 0.000431378, 2.99428e-006},
{4.10357e-005, 0.00118833, 0.000653028, 4.54095e-006},
{0.000716361, 0.000304958, 0.00112208, 7.77814e-006},
{0.000128529, 1.1673e-005, 0.000151123, 1.05138e-006}
};
	cout << ineps_err[2][2]<<endl;

	//Graphen
	TGraphErrors *dat[4];
	//Graphen Namen
	//Fitfunktion
	TF1* func = new TF1("func","12*3.14159265358979323846*[2]**2*x/([0]**2*((x-[0]**2)**2+x**2*[1]**2/[0]**2))",88.45,94.0);
	
	//tatsächliche Ereignisse, Multiplikation mit ineps
	double Nk[4][7];
	double Nk_var[4][7]; //Varianz(s^2) von Nk
	double wq[7];//7 Wq pro Zerfallsart
	double wq_err[7];
	//i-te Zerfallsart
	for(int i=0; i<4;i++){
		//für jede Schwerpunktsenergie j
		for(int j=0; j<7;j++){
			Nk[i][j]=0;
			Nk_var[i][j]=0;
			for(int l=0; l<4;l++){
				Nk[i][j]+=ineps[i][l]*N[l][j];
				Nk_var[i][j]+=ineps[i][l]**2*N[l][j]+(N[l][j]*ineps_err[i][l])**2;
				if(i==0 && l==0){
					Nk[i][j]*=factors[j];
					Nk_var[i][j]+=(ineps[i][l]*N[l][j]*factors_err[j])**2;
				}
			}
			if(i<3){wq[j]=Nk[i][j]/L[j]+wq_cor_lep[j];}
			else{wq[j]=Nk[i][j]/L[j]+wq_cor_had[j];}
			wq_err[j]=wq[j]*sqrt(Nk_var[i][j]/Nk[i][j]**2+(L_err[j]/L[j])**2);
		}
		dat[i]= new TGraphErrors(7,S,wq,0,wq_err);
		func -> SetParameters(91.187,2.4862,0.0838);
		dat[i]->SetMarkerSize(1.0);
		dat[i]->SetMarkerStyle(5);
		TCanvas* c = new TCanvas("c", "c");
		dat[i]->Draw("AP");
		func -> Draw("SAME");
		dat[i] -> Fit("func","","",88.48021,93.71841);
		//func -> Draw("SAME");
		c->SaveAs("WQ "+names[i]+".png");
		c->Close();
	}
}